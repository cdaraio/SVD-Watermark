...
def calcola_psnr(
    ...
    # Peak Signal-to-Noise Ratio tra due immagini (in dB)
    # Più alto è meglio. Ritorna inf se le immagini sono identiche
    # Soglie tipiche per il watermarking: > 30 dB (buono), > 40 dB (trasparente)
...
def calcola_ssim(
    ...
    # Structural Similarity Index (SSIM) compreso tra -1 e 1 [−1, 1]
    # più vicino a 1 significa maggiore fedeltà dell'immagine modificata rispetto all'originale.
    # NB l'immagine non viene confrontata tutta insieme ma a piccoli blocchi 
...
def calcola_nc(
    ...
    # Correlazione Normalizzata (NC) tra watermark originale ed estratto
    # valori vicini a 1 indicano alta similarita'  
...
def calcola_ber(
    ...
    # Bit Error Rate (BER) tra i watermark binarizzati
    # la relazione BER = (bit diversi) / (bit totali)  valore [0, 1]
    # le immagini vengono binarizzate con la soglia prima del confronto
