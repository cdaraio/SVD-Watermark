  ...
  return (
    <div className="max-w-5xl mx-auto px-8 py-10 animate-fade-in">
      ...
      <div className="grid grid-cols-5 gap-4 mb-6">
        ...
        {/* === MODIFICA QUI: NUOVA SEZIONE CHIAVE SEGRETA CON CARICAMENTO FILE === */}
        <div className="col-span-3 flex flex-col gap-2">
            ...
            <div>
              <input 
                ...
                onChange={(e) => {
                  ...
                  reader.onload = (evento) => setChiaveSegreta(evento.target.result);
                  reader.readAsText(file);
                }}
              />
              <label 
                htmlFor="carica-chiave" 
                className="text-[11px] font-medium text-cyber bg-cyber/10 hover:bg-cyber/20 px-2 py-1 rounded cursor-pointer transition-colors border border-cyber/20"
              >
                + Carica File .key
              </label>
            </div>
          ...
          <div className="relative flex-1">
            <textarea
              ...
              placeholder="Carica il file della chiave, oppure incolla qui il codice base64..."
              rows={4}
              className="w-full h-full resize-none rounded-xl bg-panel border border-panel-border
                text-zinc-300 text-[10px] font-mono placeholder-zinc-700 leading-tight
                px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyber/40 focus:border-cyber
                transition-all"
            />
            {chiaveSegreta.length > 0 && (
              <div className="absolute bottom-2.5 right-3 text-[10px] text-zinc-500 bg-panel px-1 rounded">
                ✔ Chiave caricata ({Math.round(chiaveSegreta.length / 1024)} KB)
              </div>
            )}
          </div>
        </div>
