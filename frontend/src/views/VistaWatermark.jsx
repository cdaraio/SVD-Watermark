...
import { ShieldCheck, Download, Check, AlertCircle, KeyRound } from "lucide-react";
...
// Valuta il livello del PSNR (sopra 40 è ottimo, sotto 30 pessimo )
...
// Valuta il livello dell'SSIM (da 0 a 1)
...
// Funzione utile per scaricare l'immagine direttamente dal browser
...
export default function VistaWatermark() {
  ...
  const [chiaveScaricata, setChiaveScaricata] = useState(false);
  ...
  // NUOVA FUNZIONE: Scarica la chiave come file .key invece di copiarla
  const scaricaChiave = () => {
    if (!risultato?.key) return;
    
    // Crea un file virtuale (Blob) con il testo della chiave
    const blob = new Blob([risultato.key], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    
    const link = document.createElement("a");
    link.href = url;
    link.download = "chiave_watermark.key";
    link.click();
    
    URL.revokeObjectURL(url);

    // Mostra il feedback visivo sul bottone
    setChiaveScaricata(true);
    setTimeout(() => setChiaveScaricata(false), 2000);
  };

  // Controlla se possiamo abilitare il bottone principale
      ...
      {risultato && !caricamento && (
        <div className="mt-8 animate-fade-in">
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            ...
            <button
              onClick={scaricaChiave}
              className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-[#111] hover:bg-[#1a1a1a] text-zinc-300 text-sm font-medium transition-colors border border-panel-border hover:border-cyber/30"
            >
              {chiaveScaricata ? (
                <>  
                  <Check className="w-4 h-4 text-cyber" />
                  <span className="text-cyber">Chiave Salvata!</span>
                </>
              ) : (
                <>
